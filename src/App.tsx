      ...
      <>
        ...
        <div className={`fixed bottom-0 left-0 right-0 bg-white rounded-t-[2rem] shadow-2xl transform transition-transform duration-300 z-50 max-h-[90vh] flex flex-col ${isCartOpen ? 'translate-y-0' : 'translate-y-full'}`}>
                          ...
                          <div className="flex-1 pr-4">
                             ...
                             {item.selectedAddons && item.selectedAddons.length > 0 && (
                                <div className="text-sm text-wood-500 mt-1 space-y-0.5">
                                   {item.selectedAddons.map((add, i) => (<div key={i} className="flex items-center gap-1 text-accent-600 font-medium"><Plus size={10} /> {add.name} (+€{add.price.toFixed(2)})</div>))}
                                </div>
                             )}
                             {(item.category === ProductCategory.HAMBURGER || item.category === ProductCategory.PIZZA) && (
                                 <button onClick={() => openAddonModal(index)} className="text-xs font-bold text-wood-400 mt-3 flex items-center gap-1 hover:text-accent-600 transition-colors border border-wood-200 rounded-lg px-3 py-1.5 w-fit"><Plus size={12}/> {t('add_ingredient', lang)}</button>
                             )}
                          </div>
           ...
           <div className="p-6 border-t border-wood-100 bg-wood-50 pb-8">
              ...
              <button className="w-full bg-wood-900 text-white py-4 rounded-2xl font-bold text-xl shadow-lg flex items-center justify-center gap-3 hover:bg-accent-600 transition-colors"><Utensils size={24} /> {t('order_table', lang)}</button>
           </div>
        </div>
        {isAddonModalOpen && (
      ...
      <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl border-4 border-wood-800 relative overflow-hidden">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div className="relative">
            ...
            <input type="password" value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} placeholder="PIN (1234)" className="w-full bg-wood-50 text-center font-mono text-2xl tracking-widest py-4 rounded-xl border-2 border-wood-100 focus:outline-none focus:border-accent-500 focus:bg-white transition-all text-wood-900" autoFocus />
          </div>
                  ...
                  {highlightedItems.map(item => {
                     ...
                     return (
                       <div key={item.id} className="snap-center shrink-0 w-48 bg-white rounded-2xl border border-wood-100 shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col" onClick={() => { setActiveCategory(item.category); document.getElementById(`btn-${item.category}`)?.click(); }}>
                          ...
                          <div className="p-3 flex flex-col flex-1"><h4 className="font-bold text-sm text-wood-900 line-clamp-2 mb-1">{name}</h4><div className="mt-auto flex justify-between items-center"><span className="font-mono font-bold text-accent-600 text-sm">€{item.price.toFixed(2)}</span><div className="bg-wood-50 p-1 rounded-full text-wood-400"><Plus size={12} /></div></div></div>
                       </div>
                     )
                  })}
                </div>
           ...
           {activeCategory === 'Tutti' ? (
             <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {CATEGORIES_LIST.map(cat => (
                   <button key={cat} onClick={() => handleCategoryClick(cat)} className="bg-white border-2 border-wood-100 hover:border-accent-500 rounded-3xl p-4 flex flex-col items-center justify-center gap-3 shadow-sm hover:shadow-xl transition-all group aspect-square">
                      <div className="w-14 h-14 bg-wood-50 rounded-full flex items-center justify-center text-wood-400 group-hover:bg-accent-50 group-hover:text-accent-500 transition-colors"><CategoryIcon category={cat} className="w-7 h-7" /></div>
